...
1. 登录后，客户端分页拉取服务端词库，并恢复用户学习状态和每日计划；客户端不再内置静态词库。
2. 服务端词库完整同步到 Room 后，Room 只作为已拉取词库的本地缓存；服务端下线的词条会从客户端缓存移除。
3. 用户在离线状态下完成学习、标记掌握或修改每日目标时，变更写入本地 mutation queue。
4. 网络恢复后，客户端以批次调用 `POST /api/learning/sync`，服务端按 `mutationId` 幂等处理。
5. 首次迁移已有本地学习数据时，客户端使用 `IMPORT_SNAPSHOT` 提交掌握次数；服务端保留更高的掌握次数。
6. 应用结束时会持久化仍在进行中的游戏会话，避免异常退出导致状态丢失。
