...
- The client and backend are maintained in this Monorepo. The Android client is
  at the repository root; the Spring Boot backend is in the `server/`
  directory.
- The remote deployment uses the Monorepo checkout
  `/home/ubuntu/coding/word-crush-app`; run backend Compose commands from its
  `/home/ubuntu/coding/word-crush-app/server` subdirectory.
- The backend is Spring Boot 3.x/Java 17 and is deployed with Docker Compose.
- The remote host is `ubuntu@txy.hejulian.org:22`; deploy from the Monorepo
  backend directory `/home/ubuntu/coding/word-crush-app/server`.
...
```powershell
ssh -i $sshKey -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=10 -p 22 ubuntu@txy.hejulian.org 'cd /home/ubuntu/coding/word-crush-app/server && docker compose --env-file .env -p wordcrush-server -f docker-compose.yml -f docker-compose.server.yml ps'
```
...
client and backend files must be committed and pushed to the Monorepo remote,
then pulled on the server. Use SCP only for an explicitly approved
...
Run backend tests and Compose validation from `server/`: `.\mvnw.cmd -q test`, `docker compose --env-file .env.example -p wordcrush-local config --quiet`, then `docker compose --env-file .env.example -p wordcrush-local up -d --build`.
...
Push the Monorepo commit to the configured Git remote before deploying. The
server deploys only the `server/` subdirectory; the Android client is not
copied into the container build context.

On the server, if the Monorepo checkout is absent, clone it into
`/home/ubuntu/coding/word-crush-app`. If it exists, run `git fetch` followed by
`git pull --ff-only` for the intended branch from the Monorepo root. Verify the
expected commit with `git rev-parse --short HEAD` and require a clean working
tree. If the remote working tree is dirty or the fast-forward pull cannot
proceed, stop and preserve it; never use `git reset --hard` as a sync shortcut.

### 3. Preflight the remote host

Only after the local gate passes, connect as `ubuntu@txy.hejulian.org` on port 22 with the configured key and one-shot non-interactive commands. Check existing containers, listeners, Docker, and the `relay_relay_internal` network. If absent, clone the Monorepo into `/home/ubuntu/coding/word-crush-app`; otherwise pull the committed Monorepo revision. Do not upload local `.env`, private keys, caches, or unrelated edits.
...
In `/home/ubuntu/coding/word-crush-app/server`, validate the merged files with `docker compose --env-file .env -p wordcrush-server -f docker-compose.yml -f docker-compose.server.yml config --quiet`, then run the same command with `up -d --build`.
...
- Use the same Compose project name and both files from the Monorepo `server/`
  directory for later status/log checks: `docker compose --env-file .env -p
  wordcrush-server -f docker-compose.yml -f docker-compose.server.yml ps` and
  `... logs --tail=100 app`.
